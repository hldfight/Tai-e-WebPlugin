...
public class CompositePlugin implements Plugin {
    ...
    private final List<Plugin> onCallWebEntryParamObjGetterPlugins = new ArrayList<>();

    private final List<Plugin> onCallMybatisMethodPlugins = new ArrayList<>();
    ...
    public void addPlugin(Plugin... plugins) {
        for (Plugin plugin : plugins) {
            ...
            addPlugin(plugin, onCallWebEntryParamObjGetterPlugins,
                    "onCallWebEntryParamObjGetter", Context.class, Invoke.class, JMethod.class);
            addPlugin(plugin, onCallMybatisMethodPlugins,
                    "onCallMybatisMethod", CSObj.class, Invoke.class);
        }
    }
    ...
    public void onCallWebEntryParamObjGetter(Context context, Invoke invoke, JMethod callee) {
        onCallWebEntryParamObjGetterPlugins.forEach(p -> p.onCallWebEntryParamObjGetter(context, invoke, callee));
    }

    @Override
    public void onCallMybatisMethod(CSObj recv, Invoke invoke) {
        onCallMybatisMethodPlugins.forEach(p -> p.onCallMybatisMethod(recv, invoke));
    }
}
